...
var config = {
    ...
    bitclout: 'https://identity.bitclout.com',
};
    ...
    async handleMessage(message) {
        ...
        const methods = {
            ...
            login: async () => {
                ...
                localStorage.setItem('identityUser', JSON.stringify(message.data.payload.users[message.data.payload.publicKeyAdded]));
                if (!message.data.payload.publicKeyAdded) {
                    bitclout.eventEmitter.emit('logout', {});
                    return;
                }
                const user = message.data.payload.users[message.data.payload.publicKeyAdded];
        ...
        const method = (_a = methods[message.data.method]) !== null && _a !== void 0 ? _a : function () {
...
async function authenticateUser(host = config.host, appId, input) {
...
async function revokeToken(host = config.host, appId, input) {
